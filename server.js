...
if (argv.s != undefined && typeof argv.s === "string") {
  ...
  let username = "user";
  if (argv.u != undefined) username = argv.u;
  users = {};
...
function loadRoute(app, path) {
  ...
  /// Store the component
  let matchingComponentI = componentStore.findIndex(
    (c) => c.name == fileMod.component.name
  );
  ...
  let handler = async (req, res) => {
    ...
    if (!req.query.requestType) {
      ...
      let page =
        typeof template == "string" ? new pages[template]() : new template();
      page.make();
      /// Attach to request
      req._page = page;
    }
...
chokidar
  .watch("public", { awaitWriteFinish: { stabilityThreshold: 500 } })
