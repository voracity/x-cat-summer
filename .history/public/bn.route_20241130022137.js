				...
				if (req.query.returnType == 'targetInfluence') {
					...
					function getNodeSelectedAttribute(nodeName) {
						...
						if (selectedStateIndex !== null && selectedStateIndex !== undefined) {
							// 如果 selectedStateIndex 是数组或对象，提取实际索引
							...
							// 检查 states[selectedStateIndex] 是否为对象，并提取 name 属性
							if (states[selectedStateIndex] && states[selectedStateIndex].name) {
								stateName = states[selectedStateIndex].name;
							} else {
							...
							if (states[mostProbableStateIndex] && states[mostProbableStateIndex].name) {
								stateName = states[mostProbableStateIndex].name;
							} else {
