				...
				if (req.query.returnType == 'targetInfluence') {
					...
					if (req.query.evidence) {
						...
						for (let nonActiveNodeName of Object.keys(evidence)) {
							...
							// Now, the network 'net' has all evidence except 'nonActiveNodeName'
							...
							// Get the baseline beliefs (with all evidence) and new beliefs (without nonActiveNodeName)
							...
							// Find all paths between the target node and the current non-active node
							...
							// After processing all paths for the current non-active node, generate the overall influence sentence
							...
							let overallSentence = `Finding out ${nonActiveNodeName} ${overallDescription} the probability that ${targetNodeName} is ${targetNodeAttribute}.`;
						
							// Combine overall influence sentence and individual contributions
							let explanation = `${overallSentence}\n\nThe contribution of each path is:\n\n`;
