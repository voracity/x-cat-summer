					...
					if (req.query.evidence) {
						...
						for (let nonActiveNodeName of Object.keys(evidence)) {
							...
							for (const path of ActivePaths) {
								...
								// 创建网络副本
								let netForPath = new Net(bnKey);
								netForPath.compile();

								// 设置路径上的证据
								for (let nodeName of path) {
									if (evidence.hasOwnProperty(nodeName)) {
										netForPath.node(nodeName).finding(Number(evidence[nodeName]));
									}
								}
								...
								// 获取目标节点的置信度
								let pathBelief = netForPath.node(targetNodeName).beliefs();
								let pathProb = pathBelief[targetStateIndex];

								// 计算差异
								console.log(baselineProb)
								console.log(pathProb)
								let diff = baselineProb - pathProb;

								console.log(`Difference for path ${path.join(' -> ')}:`, diff);

								// 存储路径和对应的diff
								if (!bn.pathInfluences) {
									bn.pathInfluences = [];
								}
								bn.pathInfluences.push({
									path: path,
									diff: diff
								});
