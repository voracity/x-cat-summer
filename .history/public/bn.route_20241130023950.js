		...
		if (req.query.updateScenario) {
			...
			try {
				...
				if (req.query.returnType == 'targetInfluence') {
					...
					// 应用证据到网络
					for (let [nodeName, stateIndex] of Object.entries(evidence)) {
						net.node(nodeName).finding(stateIndex);
						console.log(`设置节点 ${nodeName} 的 finding 为状态索引 ${stateIndex}`);
					}

					// 应用 selectedStates 到网络
					for (let [nodeName, stateIndex] of Object.entries(selectedStates)) {
						net.node(nodeName).finding(stateIndex);
						console.log(`设置节点 ${nodeName} 的 finding 为状态索引 ${stateIndex}`);
					}
					net.update();
