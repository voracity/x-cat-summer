				...
				if (req.query.returnType == 'targetInfluence') {
					...
					function calculateIndirectInfluence(path, evidence) {
						...
						for (let i = 0; i < path.length - 1; i++) {
							...
							// Get the state index for the fromNode from the evidence
							...
							// Get the baseline belief of the toNode before setting fromNode's state
							...
							// Update the network after setting fromNode's state
							tempNet.update();
					
							// Get the toNode's belief after setting the fromNode's state
							let beliefGivenParentState = toNode.beliefs();
					
							// Calculate the influence percentage by comparing beliefs
							...
							// Accumulate the total influence
