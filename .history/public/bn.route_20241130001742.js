			...
			try {
				...
				if (req.query.returnType == 'targetInfluence') {
					...
					function getNodeSelectedAttribute(nodeName) {
						...
						// Try to get the selected state index from 'evidence'
						let selectedStateIndex = evidence[nodeName];
					
						// If not found in 'evidence', try 'selectedStates'
						if (selectedStateIndex === undefined) {
							selectedStateIndex = selectedStates[nodeName] && selectedStates[nodeName][0];
						}
					
						// If still undefined, use the node's finding
						if (selectedStateIndex === undefined) {
							selectedStateIndex = node.finding();
						}
					
						// Handle cases where 'selectedStateIndex' might be an array or object
						if (Array.isArray(selectedStateIndex)) {
							selectedStateIndex = selectedStateIndex[0];
						} else if (typeof selectedStateIndex === 'object') {
							selectedStateIndex = selectedStateIndex.value || selectedStateIndex[0];
						}
					
						if (selectedStateIndex !== null && selectedStateIndex !== undefined) {
							return stateNames[selectedStateIndex];
						} else {
							// If no evidence is set, return the most probable state
