				...
				if (req.query.returnType == 'targetInfluence') {
					...
					if (req.query.evidence) {
						...
						for (let nonActiveNodeName of Object.keys(evidence)) {
							...
							// Iterate over each non-active node (evidence node)
							Object.keys(evidence).forEach(currentNonActiveNodeName => {
								// Initialize influence data for the current non-active node if not already initialized
								if (!bn.influences[currentNonActiveNodeName]) {
									bn.influences[currentNonActiveNodeName] = {
										targetBeliefs: {}
									};
								}

								// Retrieve influence data for the current non-active node
								let influenceData = bn.influences[currentNonActiveNodeName];
								...
								Object.keys(selectedStates).forEach(targetNodeName => {
									...
									const allPaths = findAllPaths(graph, currentNonActiveNodeName, targetNodeName);
									const directPaths = [];
									const indirectPaths = [];
									const sentences = [];

									console.log(`Paths from ${currentNonActiveNodeName} to ${targetNodeName}:`, allPaths.length);
									...
									if (Object.keys(evidence).length - 1 === Object.keys(evidence).indexOf(currentNonActiveNodeName)) {
