...
function mapInfluencePercentageToScale(influencePercentage) {
					...
					function calculateInfluenceBetweenNodes(parentNodeName, childNodeName,bnKey) {
					...
					function calculateIndirectInfluence(nonActiveNodeName, targetNodeName,bnKey) {
						// Create a new network instance to avoid altering the main network
						let tempNet = new Net(bnKey);
						...
						return influencePercentage * 100; // Convert to percentage
					}
					
					
					

					function calculatePathContribution(path,bnKey) {
						for (let i = 0; i < path.length - 1; i++) {
							const fromNode = path[i];
							const toNode = path[i + 1];
							const Influence = calculateIndirectInfluence(fromNode,toNode,bnKey);
				...
				if (req.query.returnType == 'targetInfluence') {
					...
					net.nodes().forEach(node => {
						...
						node.parents().forEach(parent => {
							...
							let influencePercentage = calculateInfluenceBetweenNodes(parent.name(), node.name(),bnKey);
								...
								Object.keys(selectedStates).forEach(targetNodeName => {
									...
									if (directPaths.length > 0) {
										directPaths.forEach(path => {
											...
											const toNode = path[1];
											let contribute = calculatePathContribution(path,bnKey);
									...
									if (indirectPaths.length > 0) {
										indirectPaths.forEach(path => {
											...
											let totalContribution = calculatePathContribution(path,bnKey);
