		...
		if (req.query.updateScenario) {
			...
			try {
				...
				if (req.query.returnType == 'targetInfluence') {
					...
					function calculatePathSpecificInfluence(path, evidence) {
						// 创建一个新的网络实例，避免修改主网络
						let tempNet = new Net(bnKey);
						tempNet.compile();
						...
						// 获取所有节点名称
						const allNodeNames = tempNet.nodes().map(node => node.name());
					
						// 设置对路径中非节点的证据，以阻断其他路径
						for (let nodeName of allNodeNames) {
							if (!path.includes(nodeName) && evidence.hasOwnProperty(nodeName)) {
								tempNet.node(nodeName).finding(Number(evidence[nodeName]));
							}
						}
					...
					function calculatePathContribution(path, evidence) {
						let influence = calculatePathSpecificInfluence(path, evidence);
