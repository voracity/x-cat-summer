				...
				if (req.query.returnType == 'targetInfluence') {
					...
					function getPathBeliefs(path) {
						...
						for(let i = 0; i < path.length - 1; i++) {
							...
							// Find arc between current and next node
							const arc = bn.arcInfluence.find(arc => 
								(arc.parent === currentNode && arc.child === nextNode) ||
								(arc.parent === nextNode && arc.child === currentNode)
							);
					...
					function getPathBeliefs(path) {
						// Initialize if arcInfluence doesn't exist
						if (!bn.arcInfluence) {
							bn.arcInfluence = [];
							return [];
						}
						...
						// Process each adjacent pair in path
						for (let i = 0; i < path.length - 1; i++) {
							const currentNode = path[i];
							const nextNode = path[i + 1];
							...
							const arc = bn.arcInfluence.find(a => 
								(a.parent === currentNode && a.child === nextNode) || 
