...
function calculateInfluenceBetweenNodes(Net, bnKey, parentNodeName, childNodeName) {
...
function calculateIndirectInfluence(Net, bnKey, evidence, selectedStates, nonActiveNodeName, targetNodeName) {
  // Create a new network instance to avoid altering the main network
  let tempNet = new Net(bnKey);
  tempNet.compile();
...
function calculatePathContribution(Net,bnKey, evidence, selectedStates, path) {
  let totalInfluence = 0;
  for (let i = 0; i < path.length - 1; i++) {
    const fromNode = path[i];
    const toNode = path[i + 1];
    const influence = calculateIndirectInfluence(Net, bnKey, evidence, selectedStates, fromNode, toNode);
...
function getAttribute(net, evidence, nodeName) {
