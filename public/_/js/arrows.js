var draw = {
  ...
  drawArrow: function (outputEl, from, to, opts = {}) {
    ...
    let gapWidth =
      Math.sign(to.x - from.x) * Math.sqrt(gap ** 2 / (ratio ** 2 + 1));
    let gapHeight =
      Math.sign(to.y - from.y) * Math.sqrt(gap ** 2 - gapWidth ** 2);
    marginEndX = gapWidth;
    marginEndY = gapHeight;
    if (opts.strokeWidth) {
    ...
    if (insideSvg) {
      ...
      $svg.append(
        (clickable = this.makeSvg("path", {
          d:
            ...
            (svgY - sy + firstY) +
            " L " +
            (svgX - sx + lastX - marginEndX) +
            " " +
            (svgY - sy + lastY - marginEndY),
          stroke: "transparent",
      ...
      if (isPath) {
        ...
        if (opts.isBlocked) {
          $arc.addClass("blocked");
        } else {
          $arc.removeClass("blocked");
        }
        ...
        if ($arc.length) {
          ...
          $svg
            .find(".triangle")
            .attr("d", `M${endX},${endY} l-1,-4 l10,4 l-10,4 l1,-4 Z`);
          $svg
            .find(".bar")
            .attr("d", `M${endX + arrowheadLength},${endY} v-8 v16`);
        } else {
          path = $(outputEl).attr(
            "d",
            "M " +
        ...
        let block = this.makeSvg("path", {
          d: `M${endX + arrowheadLength},${endY} v-8 v16`,
          "stroke-width": "2",
          class: "bar",
        });
        let endpoints = {};
        ...
        $svg
          .append(
            this.makeSvg(
              "g",
              { class: opts.isBlocked ? "blocked arc" : "arc", ...endpoints },
              [
                (path = this.makeSvg("path", {
                  d:
                    "M " +
                ...
                (arrow = this.makeSvg(
                  "g",
                  {
                    style: `transform: rotate(${angle}deg); transform-origin: ${endX}px ${endY}px;`,
                    class: "head",
                  },
                  [
                    this.makeSvg("path", {
  ...
  drawArrowBetweenBoxes: function (outputEl, from, to, opts = {}) {
    ...
    var fromInt = ints[0];
    if (
      this.dist(ints[1], [to.cx, to.cy]) < this.dist(ints[0], [to.cx, to.cy])
    ) {
      fromInt = ints[1];
    }
    ...
    var toInt = ints[0];
    if (
      this.dist(ints[1], [from.cx, from.cy]) <
      this.dist(ints[0], [from.cx, from.cy])
    ) {
      toInt = ints[1];
    }
  ...
  getBox: function (el) {
    ...
    let box = {
      width: elBox.width,
      height: elBox.height,
      left: elBox.left - parBox.left + scrollLeft,
      top: elBox.top - parBox.top + scrollTop,
    };
    return box;
  },
  drawArrowBetweenEls: function (outputEl, fromEl, toEl, opts = {}) {
    var path = this.drawArrowBetweenBoxes(
      outputEl,
      this.getBox(fromEl),
      this.getBox(toEl),
      opts
    ...
    $(path)
      .attr("data-can-redraw", true)
      .data("redraw", { outputEl, fromEl, toEl });
    return path;
  },
