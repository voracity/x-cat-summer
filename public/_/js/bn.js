...
var bn = {
  ...
  async guiUpdate() {
    bnDetail.$handleUpdate({
      nodeBeliefs: this.beliefs,
      influences: this.influences,
      arcInfluence: this.arcInfluence,
  ...
  guiUpdateInfoWindows() {
    ...
    if (this.roles?.cause?.length && this.roles?.effect?.length) {
      ...
      let causeStates = causes.map(
        (cause) =>
          this.selectedStates[cause] &&
          this.getNode(cause).model.states[this.selectedStates[cause]]
      );
      let effect = this.roles.effect[0];
      let effectState =
        this.selectedStates[effect] &&
        this.getNode(effect).model.states[this.selectedStates[effect]];
            ...
            Object.values(this.measureResults).map((measure) =>
              n(
                ...
                n(
                  ...
                  isNaN(measure.value)
                    ? measure.value
                    : Math.round(measure.value * 10000) / 10000,
                  measure.percent
                    ? n(
                        "span.percent",
                        " (",
                        Math.round(measure.percent * 1000) / 10,
                        "%)"
                      )
                    : "",
                  measure.extraInfo
                    ? n("span.extraInfo", measure.extraInfo)
                    : ""
                )
              )
            )
  ...
  async showCiTable() {
    ...
    for (let row of reqData.ciTable) {
      let rowClass = (this.roles?.cause ?? []).includes(row.cause)
        ? "cause"
        : row.cause == this.roles?.effect
        ? "effect"
        : "";
      ...
      table.append(
        n(
          ...
          n(
            "td.percentBar",
            { style: `--percent-bar: ${row.percent * 100}%` },
            roundedPercent
          )
        )
      );
...
class Node {
  ...
  setRole(role) {
    ...
    if (role != "cause" && this.bn.roles[role]) {
      this.bn.roles[role].forEach((nodeName) =>
        this.bn.nodes[nodeName].setRole(null)
      );
    }
  ...
  guiSetRole() {
    let removedCause =
      this.el().dataset.role == "cause" && this.role != "cause";
    if (this.role) {
  ...
  guiPopupMenu() {
    ...
    let { left, bottom } = this.el()
      .querySelector("a.menu")
...
function setupScenarioEvents() {
  ...
  saveScenario.addEventListener("click", async (event) => {
    ...
    let upd = {
      ...
      roles: bn.roles,
      selectedStates: bn.selectedStates,
      name,
    };
    let qs = getQs();
    ...
    scenarioBox.append(
      n("option", upd.name, {
        value: res.scenarioId,
        dataScenario: JSON.stringify(upd),
      })
    );
    scenarioBox.value = res.scenarioId;
  });
  removeScenario.addEventListener("click", (event) => {
    let scenarioId = event.target
      .closest(".controls")
      .querySelector(".scenario").value;
    let qs = getQs();
    fetch(
      "/bn?deleteScenario=1&requestType=data&id=" +
        qs.id +
        "&scenarioId=" +
        scenarioId,
      {
  ...
  renameScenario.addEventListener("click", (event) => {
    ...
    fetch(
      "/bn?renameScenario=1&requestType=data&id=" +
        qs.id +
        "&scenarioId=" +
        scenarioId,
      {
...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  document.querySelector(".bnView").addEventListener("click", async (event) => {
    ...
    if (target) {
      ...
      let states = [...node.querySelectorAll(".state.istarget")].map((el) =>
        Number(el.closest(".state").dataset.index)
      );
      ...
      if (!states.length) {
        ...
        Array.from(document.querySelectorAll("span.barchange")).forEach(
          (elem) => {
            elem.style.width = "";
            Array.from(elem.classList).forEach((classname) => {
        ...
        Array.from(
          document.querySelectorAll(".node span:not(.barParent)")
        ).forEach((elem) =>
          Array.from(elem.classList).forEach((classname) => {
            if (classname.indexOf("influence-") == 0)
              elem.classList.remove(classname);
    ...
    let doPublish = async (_) => {
      ...
      let res = await fetch("/bn?requestType=data&updateBn=1", {
        ...
        body: q(new FormData())
          .append(
            "updates",
            JSON.stringify({ visibility: "public", id: qs.get("id") })
          )
          .unchain(),
      });
...
document.addEventListener("DOMContentLoaded", (event) => {
  function handleDragEnter(e) {
    ...
    this.currentZoom =
      Math.trunc(((window.outerWidth - 10) / window.innerWidth) * 100) / 100;
  }
  ...
  function handleDragMove(e) {
    if (this.classList.contains("over")) {
      ...
      let matrix = new WebKitCSSMatrix(
        window.getComputedStyle(this).getPropertyValue("transform")
      );
      let moved = matrix.translate(
        e.movementX / this.currentZoom,
        e.movementY / this.currentZoom
      );
