          ...
          sortedArcInfluence.forEach((arcEntry) => {
            ...
            let arc = document.querySelector(
              `[data-child=${arcEntry.child}][data-parent=${arcEntry.parent}]`
						...
						let arcSize = 8;
						let headSize = 2;
						let arcColor = arcEntry.color;								

						if (evidenceNodeLabels.has(arcEntry.child) && arcColor != "influence-idx3" && arcEntry.child != targetNodeLabel) {
							importantMiddleNodes.add(arcEntry.parent)
						}

						if (evidenceNodeLabels.has(arcEntry.parent) && arcColor != "influence-idx3" && arcEntry.parent != targetNodeLabel) {
