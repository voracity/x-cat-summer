...
var measurePlugins = {
  ...
  ci: {
    calculate(nets, roles, selectedStates, opts = {}) {
      ...
      let effect =
        roles && roles.effect && roles.effect.length && roles.effect[0];
  ...
  cheng: {
    calculate(nets, roles, selectedStates, opts = {}) {
      ...
      if (causes && effect) {
        ...
        if (
          selectedStates[cause] &&
          selectedStates[effect] &&
          ...
          let causeStateNums = selectedStates[cause].map(
            (sel) => net.node(cause).state(sel).stateNum
          );
          let effectStateNums = selectedStates[effect].map(
            (sel) => net.node(effect).state(sel).stateNum
          );
          ...
          let cBelief = effectStateNums
            .map((e) => cBeliefs[e])
            .reduce((a, v) => a + v);
          let notCBelief = effectStateNums
            .map((e) => notCBeliefs[e])
            .reduce((a, v) => a + v);
        ...
        return {
          ...
          title: "Cheng",
          tooltip:
            "Cheng's causal power. See the CAT Explainer for a description.",
          extraInfo: preventative ? "(preventative)" : "",
        };
      }
    ...
    calculate(nets, roles, selectedStates, opts = {}) {
      ...
      if (causes && effect) {
        ...
        return {
          ...
          title: "FAR",
          tooltip:
            "Fraction of Attributable Risk. See the CAT Explainer for a description.",
          extraInfo: preventative ? "(preventative)" : "",
        };
      }
...
class BnDetail {
    ...
    this.root =
      ...
      n(
        ...
        n(
          ...
          n("span.gap"),
          n(
            "span.scenarioControls",
            n("span", "Scenario:"),
            (this.scenarioBox = n("select.scenario", n("option.none", "None"))),
        ...
        n(
          ...
          n(
            "div",
            { class: "evidence-scale" },
            n(
              "div",
              { class: "evidence-scale-header" },
              "Evidence impact scale"
            ),
            // n("div", "Colour scale showing the influence of evidence on the target."),
            n(
              "table",
              { class: "influencelegend" },
              n(
                "tr",
                n("td", "greatly increases", { class: `influence-idx0` })
              ),
              n(
                "tr",
                n("td", "moderately increases", { class: `influence-idx1` })
              ),
              n(
                "tr",
                n("td", "slightly increases", { class: `influence-idx2` })
              ),
              n("tr", n("td", "barely changes", { class: `influence-idx3` })),
              n(
                "tr",
                n("td", "slightly decreases", { class: `influence-idx4` })
              ),
              n(
                "tr",
                n("td", "moderately decreases", { class: `influence-idx5` })
              ),
  ...
  getColor(changerate) {
    if (changerate <= -0.3) return "influence-idx6";
    else if (-0.3 < changerate && changerate <= -0.15) return "influence-idx5";
    else if (-0.15 < changerate && changerate < 0) return "influence-idx4";
    else if (changerate == 0) return "influence-idx3";
    else if (0 < changerate && changerate <= 0.15) return "influence-idx2";
    else if (0.15 < changerate && changerate <= 0.3) return "influence-idx1";
    else if (0.3 < changerate) return "influence-idx0";
  }
  ...
  $handleUpdate(m) {
              ...
              n(
                ...
                n(
                  ...
                  n(
                    ...
                    n(
                      "div.hiddencheckboxcontainer.target",
                      n(
                        "input",
                        { type: "checkbox" },
                    ...
                    n(
                      "span.prob",
                      Math.trunc(Math.round(node.beliefs[i] * 100).toFixed(1))
                    )
                  )
                ),
                ...
                n(
                  ...
                  n(
                    "div.barParent",
                    n("span.bar", {
                      style: `width: ${node.beliefs[i] * 100}%`,
                    }),
                    n("span.barchange")
                  )
                )
    ...
    if (m.nodeBeliefs) {
      for (let [nodeName, beliefs] of Object.entries(m.nodeBeliefs)) {
        let nodeEl = this.bnView.querySelector(
          `div.node[data-name=${nodeName}]`
        );
        nodeEl.querySelectorAll(".state").forEach((state, i) => {
          state.querySelector(".prob").textContent = Math.trunc(
            Math.round(beliefs[i] * 100).toFixed(1)
          );
    ...
    if (m.scenariosEnabled != null) {
      this.root.querySelector(".scenarioControls").style.display =
        m.scenariosEnabled ? "inline" : "none";
    }
    if (m.scenarios) {
      for (let scenario of m.scenarios) {
        this.scenarioBox.append(
          n("option", scenario.name, {
            value: scenario.id,
            dataScenario: JSON.stringify(scenario),
          })
        );
      }
    }
    if (m.temporary != null) {
      this.root.querySelector("button.publish").style.display = m.temporary
        ? "none"
        : "inline";
      if (!m.temporary) {
    ...
    if (m.visibility != null) {
      this.root.querySelector("button.publish").style.display =
        m.visibility == "public" ? "none" : "inline";
    }

    if (m.updateFrameMode != null) {
      // clear all influences first, as we set them anyway
      Array.from(this.bnView.querySelectorAll(`span.barchange`)).forEach(
        (node) => {
          Array.from(node.classList).forEach((classname) => {
            if (classname.indexOf("influence-") == 0) {
              ...
              if (this.drawFrame) {
                ...
                if (boxidx < 0) classname = classname + "-box";
              } else {
                node.classList.remove("frame");
                if (boxidx > 0) classname = classname.substring(0, boxidx);
              }
    ...
    if (m.updateShowBarChange != null) {
      ...
      Array.from(evidencenodes).forEach((node) =>
        Array.from(node.querySelectorAll("span.barchange")).forEach(
          (e) =>
            (e.style.display = this.onlyTargetNode ? "none" : "inline-block")
        )
      );
    }
    ...
    Array.from(this.bnView.querySelectorAll(`.barchange`)).forEach(
      (node) => (node.style.width = "0%")
    );
    Array.from(this.bnView.querySelectorAll(`.cellProbability`)).forEach(
      (node) => {
        Array.from(node.classList).forEach((classname) => {
          if (classname.indexOf("influence-") == 0) {
            node.classList.remove(classname);
    ...
    if (m.influences) {
      ...
      if (entries.length == 0) {
        ...
        entries.forEach(([evidenceNodeName, value]) => {
          ...
          let evidenceNode = this.bnView.querySelector(
            `div.node[data-name=${evidenceNodeName}]`
          );
          ...
          Object.entries(targetBeliefs).forEach(([targetNodeName, beliefs]) => {
            ...
            let targetBaseModel = m.origModel.find(
              (item) => item.name == targetNodeName
            );
            listTargetNodes[targetNodeName] = {
              targetStateElem: targetStateElem,
              index: targetStateIdx,
              model: targetBaseModel,
            };
            ...
            let relativeBeliefChange =
              m.nodeBeliefs[targetNodeName][targetStateIdx] -
              beliefs[targetStateIdx];
            let absChange = Math.abs(relativeBeliefChange * 100);
            let stateElem = evidenceNode.querySelector(
              `div.state[data-index="${evidenceStateIdx}"]`
            );
            let barchangeElem = stateElem.querySelector(`span.barchange`);
            let cellProbabilityElem =
              stateElem.querySelector(`.cellProbability`);
            ...
            if (this.drawOptions.drawChangeBar) {
              ...
              barchangeElem.style.display = this.onlyTargetNode
                ? "none"
                : "inline-block";
      ...
      Object.entries(listTargetNodes).forEach(([targetNodeName, data]) => {
        ...
        let barchangeElem =
          data.targetStateElem.querySelector(`span.barchange`);
      ...
      if (this.drawOptions.drawChangeBar)
        ...
        Array.from(document.querySelectorAll(".node"))
          .filter(
            (n) =>
              !n.classList.contains("hasEvidence") &&
              !n.classList.contains("istargetnode")
          )
          .forEach((node) => {
            let nodelabel = node.getAttribute("data-name");

            let currentBelief = m.nodeBeliefs[nodelabel];
            let origBeliefs = m.origModel.find(
              (entry) => entry.name == nodelabel
            ).beliefs;
            ...
            currentBelief.forEach((curBelief, idx) => {
              ...
              let barchangeElem = node.querySelector(
                `.state[data-index="${idx}"] .barchange`
              );
              barchangeElem.classList.add(colorClass);
...
module.exports = {
  ...
  async prepareData(req, res, db, cache) {
    ...
    let userInfo = await db.get(
      "select userId from user_sessions where sessionId = ?",
      req.cookies.sessionId
    );
    console.log("x");
    if (req.query.updateScenario) {
      await db.run(
        "insert into scenarios (userId, bnId, name, evidence, roles, selectedStates) values (?, ?, ?, ?, ?, ?)",
        userInfo.userId,
        req.query.id,
        req.body.name,
        JSON.stringify(req.body.evidence),
        JSON.stringify(req.body.roles),
        JSON.stringify(req.body.selectedStates)
      );
      return {
        scenarioId: (await db.get("select last_insert_rowid() as id")).id,
      };
    } else if (req.query.deleteScenario) {
      await db.run(
        "delete from scenarios where userId = ? and id = ?",
        userInfo.userId,
        req.query.scenarioId
      );
    } else if (req.query.renameScenario) {
      await db.run(
        "update scenarios set name = ? where userId = ? and id = ?",
        req.body.name,
        userInfo.userId,
        req.query.scenarioId
      );
      ...
      let ok = await db.get(
        "select 1 from bns where userId = ? and id = ?",
        userInfo.userId,
        Number(updates.id)
      );
      ...
      if (ok) {
        ...
        db.run(
          "update bns set visibility = $visibility where id = $id",
          updParams
        );
      }
    } else {
      ...
      try {
        ...
        if (req.query.tempId) {
          ...
          bn = await db.get(
            "select name, url, visibility from bns where id = ?",
            req.query.id
          );
          bn.temporary = false;
            ...
            for (let nodeName of nodeNames) {
              if (role == "cause") {
                ...
                net
                  .node(nodeName)
                  .cpt1d(
                    Array.from({ length: orig.length }, (_) => 1 / numStates)
                  );
              }
            }
          }
        ...
        if (req.query.returnType == "targetInfluence") {
          if (req.query.evidence) {
            ...
            baselineModel = net
              .nodes()
              .map((n) => ({ name: n.name(), beliefs: n.beliefs() }));
            // origNet.update();
            ...
            for (let nonActiveNodeName of Object.keys(evidence)) {
              ...
              Object.keys(selectedStates).forEach((targetNodeName) => {
                bn.influences[nonActiveNodeName].targetBeliefs[targetNodeName] =
                  net.node(targetNodeName).beliefs();
              });
            }
            ...
            net.nodes().forEach((child) => {
              ...
              child.parents().forEach((parent) => {
                ...
                Object.keys(selectedStates).forEach((targetNodeName) => {
                  entry.targetBelief[targetNodeName] = netWithnewCPT
                    .node(targetNodeName)
                    .beliefs();
                });
          ...
          if (roles && roles.cause && roles.cause.length > 1 && hasSelStates) {
            ...
            console.log(
              "states:",
              net.node(calculateOpts.jointCause).stateNames()
            );
        ...
        if (userInfo) {
          ...
          bn.scenarios = await db.all(
            "select id, name, evidence, roles, selectedStates from scenarios where userId = ? and bnId = ?",
            userInfo.userId,
            req.query.id
          );
          /// Fix jsons
