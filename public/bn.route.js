		...
		calculate(nets, roles, selectedStates, opts = {}) {
			...
			if (causes && effect) {
				...
				return {
					value,
					title: "Cheng",
					tooltip: "Cheng's causal power. See the CAT Explainer for a description.",
		...
		calculate(nets, roles, selectedStates, opts = {}) {
			...
			if (causes && effect) {
				...
				return {
					value,
					title: "FAR",
					tooltip: "Fraction of Attributable Risk. See the CAT Explainer for a description.",
					extraInfo: preventative ? "(preventative)" : "",
				};
			}
...
class BnDetail {
	constructor(drawOptions = {}) {
		...
		Object.keys(DEFAULT_OPTS).forEach((key) => {
			if (this.drawOptions[key] == undefined) this.drawOptions[key] = DEFAULT_OPTS[key];
		});
	}
	make(root) {
		this.root =
			...
			n(
				...
				n(
					...
					n("label", "Scale Image", n("select.scaleimage", n("option", "1x", { value: 1 }), n("option", "2x", { value: 2 }), n("option", "3x", { value: 3 }))),
				...
				n("div.influenceContainer", { class: "influenceContainer" }, n("h2", "Influence Descriptions"), n("ul", { class: "influenceList" })),
	...
	$handleUpdate(m) {
							...
							n(
								...
								n(
									...
									n(
										...
										n("div.hiddencheckboxcontainer.target", n("input", { type: "checkbox" }, { class: `hiddencheckbox` })),
										n("span.label", s),
										n("span.prob", Math.trunc(Math.round(node.beliefs[i] * 100).toFixed(1)))
									)
								),

								n("div.cellBar", n("div.barParent", n("span.bar", { style: `width: ${node.beliefs[i] * 100}%` }), n("span.barchange")))
							)
						) // states.map
					)
				)
		...
		if (m.updateShowBarChange != null) {
			...
			Array.from(evidencenodes).forEach((node) =>
				Array.from(node.querySelectorAll("span.barchange")).forEach((e) => (e.style.display = this.onlyTargetNode ? "none" : "inline-block"))
			);
		}
		...
		if (m.influences) {
			...
			if (entries.length == 0) {
				...
				if (m.arcInfluence) {
					...
					const sortedArcInfluence = sortArcInfluenceByDiff(m.arcInfluence, m.nodeBeliefs, this.getColor);
					...
					sortedArcInfluence.forEach((arcEntry) => {
						...
						setTimeout(() => {
							...
							let combinedElems = Array.from(influeceArcBodyElems).map((bodyElem, index) => {
								return {
									body: bodyElem,
							...
							combinedElems.forEach((pair, index) => {
								...
								let bodyColor = getComputedStyle(document.documentElement).getPropertyValue(`--${arcEntry.color}`);
			...
			Object.entries(listTargetNodes).forEach(([targetNodeName, data]) => {
				...
				if (Math.abs(diff) > 0) barchangeElem.classList.add(colorClass + "-box");
			});
			...
			if (this.drawOptions.drawChangeBar)
				...
				Array.from(document.querySelectorAll(".node"))
					.filter((n) => !n.classList.contains("hasEvidence") && !n.classList.contains("istargetnode"))
					.forEach((node) => {
	...
	saveSnapshot() {
		...
		let dlg = ui.popupDialog([n("h2", "Enter Snapshot Name"), n("input", { id: "snapshotname", autofocus: "" })], { buttons: [btnOK, btnCANCEL] });
			...
			try {
				...
				if (req.query.returnType == "targetInfluence") {
					...
					function calculateIndirectInfluence(nonActiveNodeName, targetNodeName) {
						...
						//test
						console.log("TEST TargetNode:", targetNode);
						if (!targetNode) {
					...
					if (req.query.evidence) {
							...
							for (const path of allPaths) {
								...
								if (isDirectPath && intermediateNodes.length === 0) {
									...
									if (adjacentNonActiveNodes.length > 0) {
										...
										let intermediateNodesStr = adjacentNonActiveNodes
											.map((node) => {
												let nodeAttribute = getAttribute(node);
												return `<span style="font-weight:900; font-size:18px">${node}</span> is <span style="font-style:italic">${nodeAttribute}</span>`;
											})
						...
						if ((sentences.length = 1)) {
							...
							let start = '<span style="font-size:18px; font-weight:900">Summary: what is the finding contribute</span><br>';

							let explanation = `${start}<br>The <span style="text-decoration:underline">contribution</span> of the finding is:`;
