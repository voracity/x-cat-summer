...
class Example {
  ...
  $handleUpdate(m) {
    if (m.heading) {
      this.updateHeading(m.heading);
    }
    if (m.nodeList) {
      let nl = this.root.querySelector(".nodeList");
      m.nodeList.forEach((node) =>
        nl.append(n("li", node.title, { dataDescription: node.description }))
      );
    }
  }
  ...
  setupEvents() {
    const nodeList = this.root.querySelector(".nodeList");

    this.root.querySelector(".hello").addEventListener("input", (event) => {
      const inputValue = event.target.value;
      this.updateHeading(inputValue);
    });

    if (nodeList) {
      nodeList.addEventListener("mouseover", (event) => {
      ...
      nodeList.addEventListener("mouseout", (event) => {
        let li = event.target.closest("li");
        if (li) {
          this.hidePopup();
        }
      ...
      console.error(".nodeList not found");
    }
  }

  /// Can run on client or server
  updateHeading(heading) {
    this.root.querySelector(".heading").textContent = heading;
  }
